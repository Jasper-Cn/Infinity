...
func _process(_delta: float) -> void:
	if Global.current_popup_page == "":
		if visible:
			if Global.mouse_dragging_item == self:
				_set_color()
				z_index += 1
		...
		if Input.is_action_just_pressed("set camera"):
			if Global.control_type == 1 and is_dragging:
				_dragging()
		if Input.is_action_just_pressed(str(self_number)):
			if Global.control_type == 0:
				visible = not visible
			if Global.control_type == 1:
				if Global.mouse_dragging_item == null or Global.mouse_dragging_item == self:
...
func _tween_position_property(duration: float = 0.15) -> void:
